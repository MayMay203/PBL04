...
public interface ActivityRepository extends JpaRepository<Hoatdong,Integer> {
   ...
   List<Hoatdong> getListActivityByMyID(Integer myID);
    @Query("select h from Hoatdong h,Dangky dk where dk.phanQuyen=false and h.id = dk.maHD.id and dk.maTK.id=:id and h.tinhTrangDuyet>0")
    List<Hoatdong> getAllActivityIsMember(Integer id);

    @Query("select h from Hoatdong h, Dangky dk where dk.phanQuyen=true and h.id = dk.maHD.id and dk.maTK.id = :id and h.tinhTrangDuyet>0")
